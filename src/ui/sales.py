    ...
    def build_rows():
        ...
        for v in ventas:
            can_cancel = (
                ...
                and P.VENTAS_CANCELAR in app.controller.current_user_permissions
            )
            actions = [
                ft.IconButton(
...
async def show_new_sale(app):
    ...
    for name, field, _ in sale_field_chain:
        ...
        def _on_submit_sale(_e, _name=name):
            _advance_sale(_name)

        if isinstance(field, ft.Dropdown):
            field.on_select = _on_submit_sale
        else:
            field.on_submit = _on_submit_sale

        def _on_key_sale(_e, _name=name):
