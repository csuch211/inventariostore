"""
Extended features controller for variants, reports, push, i18n, and image search
"""
...
class ExtendedFeaturesController:
    """Extended features controller"""
    ...
    def __init__(
        ...
        logger.info("ExtendedFeatures Controller initialized")
    ...
    async def crear_variante(
        ...
        try:
            vid = extended_features_db.crear_variante(
                self.db,
            ...
            logger.exception(f"Error creating variant: {e}")
    ...
    async def obtener_variantes(
        ...
        try:
            return extended_features_db.obtener_variantes(
                self.db,
                producto_id=producto_id,
                sku=sku,
            ...
            logger.exception(f"Error fetching variants: {e}")
            return []
    ...
    async def actualizar_stock_variante(
        ...
        try:
            res = extended_features_db.actualizar_stock_variante(
                self.db,
            ...
            logger.exception(f"Error updating variant stock: {e}")
            return False, {"error": str(e)}

    @require_permission(Perm.VARIANTES_GESTIONAR)
    async def eliminar_variante(self, variante_id: int) -> tuple[bool, dict]:
        try:
            extended_features_db.eliminar_variante(
                self.db,
                variante_id=variante_id,
                usuario=self.current_user or "system",
            )
            ...
            logger.exception(f"Error deleting variant: {e}")
            return False, {"error": str(e)}
    ...
    async def guardar_plantilla_reporte(
        ...
        try:
            pid = extended_features_db.guardar_plantilla(
                self.db,
                nombre=nombre,
                modulo=modulo,
                columnas=columnas,
    ...
    async def obtener_plantillas_reporte(self) -> list[dict]:
        try:
            return extended_features_db.obtener_plantillas(self.db)
        except Exception as e:
    ...
    async def eliminar_plantilla_reporte(self, plantilla_id: int) -> tuple[bool, dict]:
        try:
            extended_features_db.eliminar_plantilla(
                self.db,
    ...
    async def ejecutar_reporte(
        ...
        try:
            return extended_features_db.ejecutar_reporte(
                self.db,
                modulo=modulo,
                columnas=columnas,
                filtros=filtros,
    ...
    async def obtener_modulos_reporte(self) -> list[dict]:
        ...
        return [{"key": k, "columns": v} for k, v in extended_features_db.REPORT_COLUMN_WHITELIST.items()]
    ...
    async def obtener_idioma_usuario(self, usuario: str) -> str:
        try:
            return extended_features_db.obtener_idioma_usuario(self.db, usuario=usuario)
        except Exception as e:
            logger.exception(f"Error fetching user lang: {e}")
            return "es"
    ...
    async def cambiar_idioma(self, usuario: str, idioma: str) -> tuple[bool, dict]:
        ...
        try:
            extended_features_db.guardar_idioma_usuario(self.db, usuario=usuario, idioma=idioma)
            ...
            logger.exception(f"Error changing lang: {e}")
            return False, {"error": str(e)}
    ...
    async def buscar_por_imagen(
        ...
        try:
            return extended_features_db.buscar_por_similitud(
                self.db,
