"""
Phase 1 controller for returns, transfers, counting, lots, prices, taxes, cash register, and KPIs
"""
...
class Phase1Controller:
    ...
    def __init__(
        ...
        logger.info("Phase1 Controller initialized")
    ...
    async def crear_devolucion(
        ...
        try:
            result = phase1_db.crear_devolucion(
                self.db,
                venta_id=venta_id,
                items=items,
                motivo=motivo,
    ...
    async def obtener_devoluciones(self, venta_id: int | None = None) -> list[dict]:
        try:
            return phase1_db.obtener_devoluciones(self.db, venta_id=venta_id)
        except Exception as e:
    ...
    async def crear_transferencia_almacen(
        ...
        try:
            result = phase1_db.crear_transferencia(
                self.db,
    ...
    async def obtener_transferencias_almacen(self, almacen_id: int | None = None) -> list[dict]:
        try:
            return phase1_db.obtener_transferencias(self.db, almacen_id=almacen_id)
        except Exception as e:
    ...
    async def crear_sesion_conteo(
        ...
        try:
            sesion_id = phase1_db.crear_sesion_conteo(
                self.db,
                nombre=nombre,
            ...
            logger.error(f"Error creating count session: {e}")
            return False, {"error": str(e)}
    ...
    async def registrar_conteo_item(
        ...
        try:
            item_id = phase1_db.registrar_conteo_item(
                self.db,
                sesion_id=sesion_id,
                producto_id=producto_id,
            ...
            logger.error(f"Error recording count: {e}")
            return False, {"error": str(e)}
    ...
    async def cerrar_sesion_conteo(
        ...
        try:
            result = phase1_db.cerrar_sesion_conteo(
                self.db,
                sesion_id=sesion_id,
            ...
            logger.error(f"Error closing count session: {e}")
            return False, {"error": str(e)}

    @require_permission(Perm.CONTEOS_LEER)
    async def obtener_sesion_conteo(self, sesion_id: int) -> dict | None:
        try:
            return phase1_db.obtener_sesion_conteo(self.db, sesion_id=sesion_id)
        except Exception as e:
    ...
    async def obtener_sesiones_conteo(self) -> list[dict]:
        try:
            return phase1_db.obtener_sesiones_conteo(self.db)
        except Exception as e:
    ...
    async def obtener_lotes(
        ...
        try:
            return phase1_db.obtener_lotes(
                self.db,
                producto_id=producto_id,
    ...
    async def eliminar_lote(self, lote_id: int) -> tuple[bool, dict]:
        try:
            phase1_db.eliminar_lote(self.db, lote_id=lote_id, usuario=self.current_user or "system")
    ...
    async def crear_lista_precio(self, nombre: str, descripcion: str = "") -> tuple[bool, dict]:
        try:
            lista_id = phase1_db.crear_lista_precio(
                self.db,
                nombre=nombre,
    ...
    async def obtener_listas_precios(self, solo_activas: bool = True) -> list[dict]:
        try:
            return phase1_db.obtener_listas_precios(self.db, solo_activas=solo_activas)
        except Exception as e:
    ...
    async def asignar_precio(
        ...
        try:
            ...
            logger.error(f"Error assigning price: {e}")
            return False, {"error": str(e)}
    ...
    async def obtener_precio_producto(
        ...
        try:
            return phase1_db.obtener_precio_producto(
                self.db,
                producto_id=producto_id,
                lista_id=lista_id,
            )
    ...
    async def actualizar_precios_producto(
        ...
        try:
            result = phase1_db.actualizar_precios_producto(
                self.db,
                producto_id=producto_id,
                precio=precio,
    ...
    async def crear_impuesto(
        ...
        try:
            tax_id = phase1_db.crear_impuesto(
                self.db,
                nombre=nombre,
    ...
    async def obtener_impuestos(self, solo_activos: bool = True) -> list[dict]:
        try:
            return phase1_db.obtener_impuestos(self.db, solo_activos=solo_activos)
        except Exception as e:
            logger.error(f"Error fetching taxes: {e}")
            return []
    ...
    async def calcular_precio_con_impuesto(
        ...
        try:
            return phase1_db.calcular_precio_con_impuesto(precio_base, porcentaje)
        except Exception as e:
    ...
    async def asignar_impuesto_producto(
        ...
        try:
            phase1_db.asignar_impuesto_producto(
                self.db,
                producto_id=producto_id,
            ...
            logger.error(f"Error assigning tax: {e}")
            return False, {"error": str(e)}
    ...
    async def abrir_turno_caja(
        ...
        try:
            turno_id = phase1_db.abrir_turno(
                self.db,
                usuario=self.current_user or "system",
    ...
    async def registrar_movimiento_caja(
        ...
        try:
            mid = phase1_db.registrar_movimiento_caja(
                self.db,
                turno_id=turno_id,
                tipo=tipo,
                monto=monto,
            ...
            logger.error(f"Error registering cash movement: {e}")
            return False, {"error": str(e)}

    @require_permission(Perm.CAJA_GESTIONAR)
    async def asociar_venta_a_turno(self, turno_id: int, venta_id: int) -> bool:
        try:
            return phase1_db.asociar_venta_a_turno(self.db, turno_id, venta_id)
        except Exception as e:
            logger.error(f"Error linking sale to shift: {e}")
            return False
    ...
    async def obtener_turno_caja(self, turno_id: int) -> dict | None:
        try:
            return phase1_db.obtener_turno(self.db, turno_id=turno_id)
        except Exception as e:
    ...
    async def obtener_turnos_caja(self, usuario: str | None = None) -> list[dict]:
        try:
            return phase1_db.obtener_turnos(self.db, usuario=usuario)
        except Exception as e:
    ...
    async def obtener_turno_abierto(self, usuario: str) -> dict | None:
        try:
            return phase1_db.obtener_turno_abierto(self.db, usuario=usuario)
        except Exception as e:
            logger.error(f"Error fetching open shift: {e}")
            return None
    ...
    async def obtener_kpis_dashboard(self) -> dict:
        try:
            return phase1_db.obtener_kpis_dashboard(self.db)
        except Exception as e:
